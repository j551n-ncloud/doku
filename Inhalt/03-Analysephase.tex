...
Aus dem Projektantrag und der Ist-Analyse ergeben sich folgende
Anforderungen an die neue Backup-Lösung:
\begin{itemize}
    \item Dedizierter \acl{PBS} auf eigener Hardware, getrennt von
          Primär-Storage und produktiven Hypervisor.
    \item Native Sicherung aller relevanten \acp{VM} und Container
          des \ac{PVE}-Clusters mit Job-Plänen und Retention je
          \ac{VM}-Klasse.
    \item Restore einzelner Dateien sowie ganzer \acp{VM} bzw.\
          Container.
    \item E-Mail-Benachrichtigung und Anbindung an das vorhandene
          \ac{Prometheus}-Monitoring.
    \item Authentifizierung über das \ac{DKFZ}-\ac{LDAP} plus
          lokalem \texttt{root}-Account; Einbindung in die bestehende
          Netzwerkstruktur (\emph{server}/\emph{cluster}/\emph{external}/
          \emph{ceph}-\ac{VLAN}).
