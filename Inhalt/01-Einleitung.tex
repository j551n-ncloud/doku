...
heterogen aufgebaut: Die virtuellen Maschinen und Container werden
lokal auf einem \ac{Ceph}-\ac{RBD}-Pool gesichert, zusätzlich werden
Daten einzelner \acp{VM} per \texttt{rsync} auf einen separaten
Backup-Server übertragen und von dort über \ac{TSM} auf Tape
(Bandlaufwerk) ausgelagert.
...
Die Speicherung der Sicherungen auf dem produktiven \ac{Ceph}-Cluster
ist zudem nicht ideal, da Primär- und Sicherungsdaten dadurch auf
derselben Storage-Infrastruktur liegen.
Ziel ist daher der Wechsel auf eine dedizierte Backup-Lösung, die
unabhängig vom \ac{Ceph}-Cluster betrieben wird und eine einheitliche,
zentral verwaltete Sicherung aller virtualisierten Dienste ermöglicht.
