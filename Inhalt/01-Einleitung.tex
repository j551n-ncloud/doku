...
\subsection{Vorstellung der DKFZ}
\label{sec:Vorstellung_DKFZ}
Die ODCF (Omics IT and Data Management Core Facility) des Deutschen Krebsforschungszentrums (DKFZ) betreibt eine Proxmox VE (Virtual
Environment) Cluster-Umgebung, die eine Vielzahl virtueller Maschinen (VMs) und Linux Container (LXC) für Forschungs- und Infrastrukturaufgaben
bereitstellt.
Eine grundlegende Backup-Infrastruktur ist vorhanden: Sicherungen der virtuellen Maschinen werden aktuell auf Hypervisor-Ebene als
Snapshots erstellt und anschließend auf Tape (Bandlaufwerk) gesichert. Ein zentralisierter, dedizierter Proxmox Backup Server (PBS) für die
Verwaltung, Überwachung und eﬀiziente inkrementelle Datensicherung aller VMs und Container existiert jedoch nicht.
Dies führt zu eingeschränkter Wiederherstellbarkeit, fehlender zentraler Verwaltung der Backup-Jobs sowie begrenzten Möglichkeiten zur Überprüfung der Backup-Integrität.
\subsection{Projektumfeld}
\label{sec:Projektumfeld}
Durchgeführt wird das Projekt von mir in den Räumen des DKFZ, bei der ich meine Ausbildung absolviere.
Das Projekt wird im Auftrag von Frank Thommen durchgeführt, die in der Herstellung von Wasserpumpen tätig ist.
