...
Das \ac{DKFZ} in Heidelberg ist die größte
...
Innerhalb des DKFZs stellt die \ac{ODCF} den Forschungsgruppen eine
komplexe IT-Infrastruktur zur Verarbeitung großer Mengen genomischer
Daten bereit.
...
Es handelt sich um ein \textbf{internes Projekt} der \ac{ODCF}; sowohl
Auftraggeber als auch Auftragnehmer sind Teil des \ac{DKFZ}, eine
externe Vergabe findet nicht statt.
Das Projekt wird in den Räumlichkeiten der \ac{ODCF} am \ac{DKFZ}~\citep{DKFZWebsite} in Heidelberg
...
auf den \emph{lokalen} Storages der älteren Hypervisor-Knoten, im
Zusammenspiel mit Ceph~\citep{CephDocs} als Primärspeicher,
...
unabhängig vom Ceph-Cluster sowie vom lokalen Storage der
...
Mit einem dedizierten \acl{PBS} können Sicherungen häufiger,
effizienter (Deduplizierung, inkrementelle Backups) und mit
...
\textbf{Referenzkonfiguration (Blueprint-Testinstanz)} des \acl{PBS} samt Anbindung an die
bestehende \ac{PVE}-Umgebung der \ac{ODCF} aufgebaut.
Diese Referenzkonfiguration dient dem \ac{ODCF}-Team als Vorlage für
